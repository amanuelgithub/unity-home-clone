...
export function Navbar() {
  const [isAllDocHavored, setIsAllDocHavored] = useState(false);

  const handleAllDocsMenuTrue = () => {
    setIsAllDocHavored(true);
  };

  const handleAllDocsMenuFalse = () => {
    setIsAllDocHavored(false);
  };

  return (
    <div className="fixed w-full z-50">
      <div className="flex justify-between items-center px-6 bg-black text-white">
        <div className="flex justify-start items-center gap-4">
          ...
          <div className="text-xl">Unity Documentation</div>
        </div>
          ...
          <div className="lg:flex gap-5 hidden">
            <div className="relative">
              <a
                ...
                onMouseOver={handleAllDocsMenuTrue}
                onMouseOut={handleAllDocsMenuFalse}
              >
                All Docs
              </a>
              <Menu
                isHoverd={isAllDocHavored}
                handleSetTrue={handleAllDocsMenuTrue}
                handleSetFalse={handleAllDocsMenuFalse}
              />
            </div>
            <NavItem href={`#`}>Learning</NavItem>
            <NavItem href={`#`}>Community</NavItem>
            <NavItem href={`#`}>{`Support & Services`}</NavItem>
          </div>
